...
species_data_study <- redfox_studyarea
...
# SCENARIO NAME
scenario_name <- "Targeted 50/50, occupancy (fixed total visits)"
scenario_ID <- "T50_3R"
...
allocate_visits <- function(n_sites,
  ...
  # enforce cap_visits (feasible because of earlier check)
...
#----------------#50/50 spatial bias........-------------
sample_cells_corridor_capped <- function(df, n_samples, corridor_col = "corridor_flag") {
  idx_in  <- which(!is.na(df[[corridor_col]]) & df[[corridor_col]] == 1)
  idx_out <- which(!is.na(df[[corridor_col]]) & df[[corridor_col]] == 0)
  
  if (length(idx_in) == 0) stop("No corridor cells (corridor_flag==1).")
  if (length(idx_out) == 0) stop("No non-corridor cells (corridor_flag==0).")
  
  # Desired split (as close to 50/50 as possible)
  n_in_desired <- n_samples %/% 2
  
  # Cap by availability
  n_in  <- min(n_in_desired, length(idx_in))
  n_out <- n_samples - n_in
  
  if (n_out > length(idx_out)) {
    stop("Not enough non-corridor cells: need ", n_out, " but have ", length(idx_out),
         ". Reduce n_samples or allow sampling with replacement.")
  }
  
  c(
    sample(idx_in,  size = n_in,  replace = FALSE),
    sample(idx_out, size = n_out, replace = FALSE)
  )
}
...
run_one_sim <- function(species_data_study,
  ...
  sampled_cells <- sample_cells_corridor_capped(
    df = species_data_study,
    n_samples = n_samples,
    corridor_col = "corridor_flag"
  )
...
saveRDS(
  scenario_perf,
  sprintf("saved/occupancy/5050/%s_%s_scenario_summary_%s.rds",
