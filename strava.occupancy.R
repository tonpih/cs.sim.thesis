...
plot_df <- bind_rows(
  moose_studyarea   %>% mutate(species = "Moose"),
  redfox_studyarea  %>% mutate(species = "Red fox"),
  roedeer_studyarea %>% mutate(species = "Roe deer")
) %>%
  filter(studyarea != 0) %>%        
  filter(!is.na(p_true_minus02))


ggplot(plot_df, aes(x = p_true_minus02, fill = species, colour = species)) +
  ...
  labs(
    title = "Distribution of predicted occurrence within the study area",
    x = "Predicted probability of occupancy",
    y = "Density",
    fill = "Species",
    colour = "Species"
  ) +
...
# Study area only (studyarea != 0)
moose_sa <- moose_studyarea
# Corridor subset 
corridor_sf <- moose_sa %>%
  dplyr::filter(studyarea == 2)

# Color scale range 
rng_occ <- range(moose_sa$p_true_adj, na.rm = TRUE)

moose_p_occ_linear2 <- ggplot(moose_sa) +
  geom_sf(aes(fill = p_true_adj), color = NA) +
  ...
  coord_sf(datum = NA) +
  labs(
    title = "Heatmap of predicted occurence: Moose",
    subtitle = "Study area with corridor cells outlined in black"
  ) +
  theme_minimal() +
...
#ROE DEER----
# Study area only
roedeer_sa <- roedeer_studyarea

# Corridor subset 
corridor_sf <- roedeer_sa %>%
  dplyr::filter(studyarea == 2)

# Color scale range 
rng_occ <- range(roedeer_sa$p_true_adj, na.rm = TRUE)

roedeer_p_occ_linear2 <- ggplot(roedeer_sa) +
  geom_sf(aes(fill = p_true_adj), color = NA) +
  scale_fill_viridis_c(
    option = "plasma", direction = -1,
    name = "Predicted probability",
  ...
  coord_sf(datum = NA) +
  labs(
    title = "Heatmap of predicted occurence: Roe deer",
    subtitle = "Study area with corridor cells outlined in black"
  ) +
  theme_minimal() +
  theme(panel.grid.major = element_line(color = "grey90"))
...
#RED FOX----
# Study area only 
redfox_sa <- redfox_studyarea

# Corridor subset 
corridor_sf <- redfox_sa %>%
  dplyr::filter(studyarea == 2)

# Color scale range 
rng_occ <- range(redfox_sa$p_true_adj, na.rm = TRUE)

redfox_p_occ_linear2 <- ggplot(redfox_sa) +
  geom_sf(aes(fill = p_true_adj), color = NA) +
  scale_fill_viridis_c(
    option = "plasma", direction = -1,
    name = "Predicted probability",
  ...
  coord_sf(datum = NA) +
  labs(
    title = "Heatmap of predicted occurence: Red fox",
    subtitle = "Study area with corridor cells outlined in black"
  ) +
  theme_minimal() +
  theme(panel.grid.major = element_line(color = "grey90"))

redfox_p_occ_linear2 +
  theme(
    text = element_text(size = 12),
    plot.title = element_text(size = 16, face = "bold"),
    plot.subtitle = element_text(size = 13),
...
#---------------- Join strava mean activity to species data: species data = SPECIES_STRAVA -------------------------
#---MOOSE------
library(dplyr)
Grid_strava <- st_read("Grid_strava.gpkg")
moose_studyarea <- moose_studyarea %>% st_transform(st_crs(Grid_strava)) #align CRS


#Spatial join: species left, grid right 
moose_strava <- st_join(
  moose_studyarea,
  dplyr::select(Grid_strava, mean_activity),
  left = TRUE,
  largest = TRUE
) %>%
  mutate(mean_activity = dplyr::coalesce(mean_activity, 0))
...
library(dplyr)
Grid_strava <- st_read("Grid_strava.gpkg")
roedeer_studyarea <- roedeer_studyarea %>% st_transform(st_crs(Grid_strava)) #align CRS


#Spatial join: species left, grid right 
roedeer_strava <- st_join(
  roedeer_studyarea,
  dplyr::select(Grid_strava, mean_activity),
  left = TRUE,
  largest = TRUE
...
library(dplyr)
Grid_strava <- st_read("Grid_strava.gpkg")
redfox_studyarea <- redfox_studyarea %>% st_transform(st_crs(Grid_strava)) #align CRS


#Spatial join: species left, grid right 
redfox_strava <- st_join(
  redfox_studyarea,
  dplyr::select(Grid_strava, mean_activity),
  left = TRUE,
  largest = TRUE
...
#--------------SIMULATION SETUP (fixed total visit budget, baseline sampling) --------------------------------------
...
n_reps <- 1000 # replicates per scenario

# NEW: total visit budgets per sample size
visit_budget_map <- c(
  "200"  = 400,
  "300" = 600,
  "400"  = 800,
...
#--- DEFINE COVARIATES: (occupancy model)
state_covariates <- c(
  "corridor_flag",
  "building_c",
  "road_area",
...
#prepare species data ----------> CHANGE SPESIES HERE:species_strava
species_data_study <- moose_strava
species_name <- "Moose"

# set species specific seed
seed_map <- c("Moose" = 100, "Roe deer" = 200, "Red fox" = 300)
set.seed(seed_map[[species_name]])

# removing sf class - for occupancy model
...
#SCENARIO NAME
scenario_name <- "Strava, occupancy"
scenario_ID <- "S11R"

#STRAVA: set probability vector - specific for strava scenario: 
baseline_bias <- 1   
prob_vector <- baseline_bias + species_data_study$mean_activity
...
# -------------------------------
# Visit allocation with fixed total visits (budget)
# -------------------------------
allocate_visits <- function(n_sites,
  ...
  # weights for distributing the remaining visits (NULL => equal)
  ...
  # allocate remaining visits (random, unequal)
...
#-------- SIMULATION FUNCTION -----------
run_one_sim <- function(species_data_study,
                        n_samples,
                        total_visits,
                        p_detect,
                        state_covariates,
  ...
  # --- 1) sample unique cells WITH STRAVA BIAS (prob_vector) ---
  
  # Align + sanitize prob_vector
  if (length(prob_vector) != nrow(species_data_study)) {
    stop("prob_vector length (", length(prob_vector),
         ") must equal nrow(species_data_study) (", nrow(species_data_study), ").")
  }
  w <- prob_vector
  w[!is.finite(w)] <- 0
  w[w < 0] <- 0
  
  if (sum(w) <= 0) stop("prob_vector has no positive finite weights after cleaning.")
  w <- w / sum(w)
  ...
  sampled_cells <- sample(seq_len(nrow(species_data_study)),
                          size = n_samples,
                          replace = FALSE,
                          prob = w)
  
  sampled_data <- species_data_study[sampled_cells, , drop = FALSE]
  
  # --- 2) simulate true occupancy state z ---
  sampled_data$z_true <- rbinom(n_samples, 1, prob = sampled_data$p_true_adj)
  
  # --- 3) random unequal effort with FIXED TOTAL VISITS (budget) ---
  # weights=NULL -> remaining visits allocated uniformly at random among sites
  K_i <- allocate_visits(
    n_sites = n_samples,
  ...
  ii <- rep.int(seq_len(n_samples), times = K_i) # row index for each visited occasion
  jj <- sequence(K_i)                            # col index 1..K_i per row
  ...
  sampled_data$K_visits <- K_i
  sampled_data <- cbind(sampled_data, as.data.frame(y_mat))
  ...
  fit <- tryCatch(
    unmarked::occu(full_form, data = umf),
    error = function(e) e
  )
...
# -------------------------------
# Run scenarios (pass total_visits per n_samples)
# -------------------------------
results <- list()
idx <- 1
...
for (p_detect in p_detect_vals) {
  for (n_samp in n_samples_vals) {
    ...
    results[[idx]] <- data.frame(
      ...
      # sampling summaries
      mean_sampled_inside   = mean(n_sampled_inside_vec),
      mean_sampled_outside  = mean(n_sampled_outside_vec),
      mean_detected_inside  = mean(n_detected_inside_vec),
      mean_detected_outside = mean(n_detected_outside_vec),
...
saveRDS(
  scenario_perf,
  sprintf("saved/occupancy/strava/%s_%s_scenario_summary_%s.rds",
