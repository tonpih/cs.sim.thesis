...
#load target areas
target_areas <- st_read("target_areas.shp")
...
n_samples_vals <- c(200, 300, 400, 600, 795)
n_reps <- 1000
...
# ---- build in_target flag using polygons (sf) ----
target_areas_33   <- st_transform(target_areas, st_crs(species_data_study_sf))
target_areas_buff <- st_buffer(target_areas_33, dist = 50)

species_data_study_sf$in_target <- apply(
  st_intersects(species_data_study_sf, target_areas_buff, sparse = FALSE),
  1, any
)
print(table(species_data_study_sf$in_target))
...
missing_covs <- setdiff(c(cont_covs, "p_true_adj", "in_target"), names(species_data_study))
...
# ---- one replicate ----
run_one_sim_target_mix <- function(species_data_study,
                                   n_samples,
                                   total_visits,
                                   p_detect,
                                   state_covariates,
                                   target_frac = 0.8,
                                   cap_visits = 20) {
  idx_target <- which(!is.na(species_data_study$in_target) & species_data_study$in_target)
  if (length(idx_target) == 0) stop("No targeted cells (in_target == TRUE).")
  
  idx_all <- seq_len(nrow(species_data_study))
  
  # counts
  n_target <- as.integer(round(n_samples * target_frac))
  n_target <- max(0L, min(n_target, n_samples))
  n_rand   <- n_samples - n_target
  
  if (n_target > length(idx_target)) {
    stop("n_target exceeds number of targeted cells: ", n_target, " > ", length(idx_target))
  }
  
  # sample UNIQUE sites
  picked_target <- if (n_target > 0) sample(idx_target, size = n_target, replace = FALSE) else integer(0)
  
  # 20% random from ALL study cells; to keep total unique sites == n_samples,
  # sample from "all minus already picked_target"
  idx_all2 <- setdiff(idx_all, picked_target)
  if (n_rand > length(idx_all2)) stop("Not enough remaining cells to draw random sites.")
  
  picked_rand <- if (n_rand > 0) sample(idx_all2, size = n_rand, replace = FALSE) else integer(0)
  
  sampled_idx <- c(picked_target, picked_rand)
  stopifnot(length(sampled_idx) == n_samples)
  ...
  umf <- unmarkedFrameOccu(y = y_mat, siteCovs = site_covs)
  
  state_rhs <- paste(state_covariates, collapse = " + ")
  full_form <- as.formula(paste("~ 1 ~", state_rhs))
  
  fit <- tryCatch(occu(full_form, data = umf), error = function(e) e)
  ...
  extract <- tryCatch({
    state_coef <- coef(fit, type = "state")
    state_vcov <- vcov(fit, type = "state")
    ...
    alpha_hat <- coef(fit, type = "det")[1]
    p_hat <- plogis(alpha_hat)
    
    list(
      beta_hat = est, se = unname(se),
      ci_low = unname(ci[1]), ci_high = unname(ci[2]),
      p_hat = unname(p_hat),
      fit_failed = FALSE, fit_error = NA_character_
    )
  }, error = function(e) {
    list(
      beta_hat = NA_real_, se = NA_real_, ci_low = NA_real_, ci_high = NA_real_,
...
# ---- run scenario grid ----
scenario_ID <- "T13R"
scenario_name <- "Targeted sampling, occupancy (random visits; fixed budget)"
...
for (p_detect in p_detect_vals) {
  for (n_samp in n_samples_vals) {
    ...
    top_error <- if (n_failed > 0) names(sort(table(fit_error_vec[fit_failed_vec]), decreasing = TRUE))[1] else NA_character_
...
saveRDS(
  scenario_perf,
  sprintf("saved/occupancy/targeted/%s_%s_scenario_summary_%s.rds",
...
# 1. Extract centroids from polygons
species_centers <- species_data_study_sf %>%
  st_centroid()

coords <- st_coordinates(species_centers)
species_centers$x <- coords[,1]
species_centers$y <- coords[,2]

# 2. Determine cell size from shape area
res <- sqrt(unique(species_centers$Shape_Area))

# 3. Plot with subtitle + color code explanation
ggplot() +
  
  # --- Corridor cells (bottom layer, dark green) ---
  geom_tile(
    data = species_centers %>% filter(corridor_flag == 1),
    aes(x = x, y = y),
    fill = "#4DAF4A",          # dark green
    width = res, height = res,
    alpha = 1.0
  ) +
  
  # --- Target cells (transparent overlay, orange) ---
  geom_tile(
    data = species_centers %>% filter(in_target == TRUE),
    aes(x = x, y = y),
    fill = "#FF8C00",          # orange
    width = res, height = res,
    alpha = 0.7
  ...
  # --- Other cells (light grey background) ---
  geom_tile(
    data = species_centers %>% 
      filter(!in_target & corridor_flag != 1),
    aes(x = x, y = y),
    fill = "grey90",
    width = res, height = res,
    alpha = 1.0
  ...
  labs(
    title = "Targeted sampling - suggested areas",
    subtitle = "Target cells (orange) and corridor cells (green)",
    x = "",
    y = ""
  )
